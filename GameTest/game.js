     ...
     generate() {
         ...
         for (let x = 1; x < this.columns - 1; x++) {
             for (let y = 1; y < this.rows - 1; y++) {
                 ...
                 if ((this.cells[x][y] == 1) && (neighbors >=  4)) 
                 {
                 	let chanceOfDyingOvercrowded = 0.8;
                 	let chanceRollOvercrowded = Math.random();
                 	if(chanceRollOvercrowded <= chanceOfDyingOvercrowded)
                 	{
                 		nextgen[x][y] = 0;
                 	}
                 	else
                 ...
                 else if ((this.cells[x][y] == 1) && (neighbors <=  1)) 
                 {
                 	let chanceOfDyingLonliness = 0.6;
                 	let chanceRollLonliness = Math.random();
                 	if(chanceRollLonliness < chanceRollLonliness)
                 	{
                 		nextgen[x][y] = 0;
                 	}
                 	else
